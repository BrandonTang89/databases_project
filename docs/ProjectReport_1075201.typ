...
Finally, we note that another straightforward optimisation is improving the efficiency of the hash indices. We currently use `std::unordered_map` with `uint32_t` which uses separate chaining and can have poor cache performance. A more cache-friendly hash table design (e.g., open-addressing with linear probing) would likely improve performance, especially for larger datasets. This can easily be achieved by using a third-party hash map implementation as Abseil's `flat_hash_map` or writing our own hash map, but this is outside the scope of the project.