...
Stage::Stage(size_t stage_index, Transaction &trx)
  ...
  if (left_is_const && right_is_const) {
    ...
    todo("handle variable-variable case");
    assert(false && "variable-variable case not implemented yet");
  }
}
...
PipelineStatus Stage::next() {
  ...
  case StageType::SINGLE_PRODUCT:
    return next_single_product();
  case StageType::DIAG_FILTER:
    assert(false && "DIAG_FILTER not implemented yet");
    // return execute_diag_filter();
  case StageType::MEMBER_FILTER:
    assert(false && "MEMBER_FILTER not implemented yet");
    // return execute_member_filter();
...
PipelineStatus Stage::next_group_filter() {
  while (true) {
    ...
    if (left_is_const && right_is_var) {
      if (group->find(const_val, output[var_idx])->alive) {
        // passes the filter
        return PipelineStatus::OK;
      }
    } else if (left_is_var && right_is_const) {
      if (group->find(output[var_idx], const_val)->alive) {
        // passes the filter
        return PipelineStatus::OK;
      }
    } else {
      assert(false && "invalid stage type for group filter");
    }
  }
  assert(false && "unreachable");
}

PipelineStatus Stage::next_single_product() {
  if (num_input_vars == 0) {
    PipelineStatus st = previous->next();
    if (st != PipelineStatus::OK) {
      return st;
    ...
    group_iter++;
    output[0] = left_is_const ? tp->right : tp->left;
    return PipelineStatus::OK;
  }
